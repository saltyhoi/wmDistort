...
p.autosave_file = strrep(p.filename, '.mat', '_AUTOSAVE.mat');
if p.do_et == 1
...
p.shapes = {'left_wide_rect', 'right_wide_rect', 'diag45_rect', 'diag135_rect'};
...
filename = sprintf('../../data/wmDistort_data/fixed_conditions_pilot5.mat');  % Define filename for the fixed file
...
if ~exist(filename, 'file')
    ...
    for shape_index = 1:p.r_cond
        for degree_index = 1:p.repetitions
            ...
            conditions{index, 3} = fix((index-1)/p.repetitions) + 5; 
            % +5 instead of +1 since I want conditions to go 5,6,7,8 
            % Numeric values used to distinguish aperture conditions 
            % 5:left_wide, 6:right_wide, 7:rect45, 8:diag135
            index = index + 1;
        end
    end
...
aperture_wide_rect_px = [0 0 H*phi H]; %remember, this is actually dimensino for tall rect
aperture_tall_rect_px = [0 0 H H*phi]; %remember, this is actually dimensino for wide rect

% Center the old apertures (kept for compatibility/debug)
...
p.aperture_wide_rect = CenterRectOnPoint(aperture_wide_rect_px(1:4), p.center(1), p.center(2)); %remember, this is actually dimensino for tall rect
p.aperture_tall_rect = CenterRectOnPoint(aperture_tall_rect_px(1:4), p.center(1), p.center(2)); %remember, this is actually dimensino for wide rect

% pilot5 apertures
p.x_offset_deg = 4;
p.offset_px = p.x_offset_deg * p.ppd;

p.aperture_rect_left = CenterRectOnPoint(aperture_tall_rect_px(1:4) ,p.center(1) - p.offset_px,p.center(2));
p.aperture_rect_right = CenterRectOnPoint(aperture_tall_rect_px(1:4) ,p.center(1) + p.offset_px,p.center(2));

p.diag_scale = 0.85;   % safe starting point otherwise, corner goes out of monitor

rect_w = H * p.diag_scale;        % long side
rect_h = H * phi * p.diag_scale;  % short side

p.aperture_diag45_rect  = makeRotRect(p.center(1), p.center(2), rect_w, rect_h, 45);
p.aperture_diag135_rect = makeRotRect(p.center(1), p.center(2), rect_w, rect_h, 135);
...
for tt = 1:p.ntrials
    ...
    if isfield(p,'debug') && p.debug
        ...
        switch current_shape
            ...
            case 'left_wide_rect',  apField = 'p.aperture_rect_left';
            case 'right_wide_rect', apField = 'p.aperture_rect_right';
            case 'diag45_rect',     apField = 'p.aperture_diag45_rect';
            case 'diag135_rect',    apField = 'p.aperture_diag135_rect';
    ...
    while GetSecs < trial_start + p.aperture_dur
        ...
        switch current_shape
            ...
            case 'left_wide_rect'
                Screen('FillRect', w, p.bg_color, p.aperture_rect_left);
            case 'right_wide_rect'
                Screen('FillRect', w, p.bg_color, p.aperture_rect_right);
            case 'diag45_rect'
                Screen('FillPoly', w, p.bg_color, p.aperture_diag45_rect');
            case 'diag135_rect'
                Screen('FillPoly', w, p.bg_color, p.aperture_diag135_rect');
...
function pts = makeRotRect(cx, cy, w, h, angle_deg)
    base = [-w/2, -h/2;
             w/2, -h/2;
             w/2,  h/2;
            -w/2,  h/2]';
